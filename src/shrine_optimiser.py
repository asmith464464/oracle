...
class ShrineOptimiser:
    """Handles shrine insertion logic and optimisation."""
    ...
    def optimise_shrine_placement(
        self,
        route: List[str],
        max_shrines: int = 3,
    ) -> Tuple[List[str], List[str]]:
        """
        Optimise shrine placement in the given route.
        ...
        Returns:
            Tuple of (optimised_route, shrine_positions)
        """
        ...
        # Analyse route for wasted moves and opportunities
        ...
        optimised_route, shrine_positions = insert_shrines_into_route(
            self.grid,
            self.distance_calc,
            route,
            selected_shrines,
        )
        optimised_route = repair_route(self.grid, self.distance_calc, optimised_route)
        ...
        if len(shrine_positions) < target_shrines:
            ...
            optimised_route, additional_shrines = add_remaining_shrines(
                self.grid,
                self.distance_calc,
                optimised_route,
                shrine_candidates,
        ...
        optimised_route = ensure_return_to_zeus(self.grid, self.distance_calc, optimised_route)

        return optimised_route, shrine_positions